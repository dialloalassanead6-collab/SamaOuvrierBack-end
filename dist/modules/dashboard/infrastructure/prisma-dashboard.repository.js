...
export class PrismaDashboardRepository {
     ...
     */
    async getWorkerMissionStats(workerId) {
        ...
        const missionsInProgress = (byStatus[MissionStatus.PENDING_PAYMENT] ?? 0) +
     ...
     */
    async getWorkerRevenueStats(workerId) {
     ...
     */
    async getWorkerReputationStats(workerId) {
     ...
     */
    async getWorkerDisputeStats(workerId) {
    ...
    async getUserStats(filters) {
        ...
        const [total, clients, workers, workersPending, workersApproved, workersRejected, activeUsers, bannedUsers, newUsersInPeriod] = await Promise.all([
     ...
     */
    async getRevenueTimeline(filters, groupByDay) {
        const periodCondition = filters.startDate && filters.endDate
            ? { createdAt: { gte: filters.startDate, lte: filters.endDate } }
            : {};
     ...
     */
    async getMissionsTimeline(filters, groupByDay) {
        const periodCondition = filters.startDate && filters.endDate
            ? { createdAt: { gte: filters.startDate, lte: filters.endDate } }
            : {};
     ...
     * Utilise groupBy pour obtenir le décompte par status
     */
    async getClientMissionStats(clientId) {
        // Requête parallèle pour les stats de base
        const [totalCount, statusGroups] = await Promise.all([
     ...
     */
    async getClientSpendingStats(clientId) {
        // Obtenir le premier jour du mois courant
        const now = new Date();
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
        // Requêtes parallèles
     ...
     */
    async getClientReliabilityStats(clientId) {
        // Requêtes parallèles
     ...
     */
    async getClientReviewsStats(clientId) {
