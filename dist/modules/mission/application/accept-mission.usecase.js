...
export class AcceptMissionUseCase {
     ...
     */
    async execute(missionId, workerId) {
        ...
        if (!mission.isPendingAccept()) {
            throw BusinessErrors.badRequest(`La mission n'est pas en attente d'acceptation. Statut actuel: ${mission.status}`);
        }
        // Utiliser l'entité pour la transition de statut
        const updatedMissionEntity = mission.acceptMission();
        // Sauvegarder en base de données
        const updatedMission = await this.missionRepository.update(missionId, updatedMissionEntity);
...
//# sourceMappingURL=accept-mission.usecase.js.map