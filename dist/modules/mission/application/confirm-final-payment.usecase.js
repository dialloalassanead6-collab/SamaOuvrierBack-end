...
export class ConfirmFinalPaymentUseCase {
    ...
    async execute(missionId, userId) {
        ...
        const canConfirmFromStatus = mission.status === MissionStatus.AWAITING_FINAL_PAYMENT ||
            (mission.status === MissionStatus.NEGOTIATION_DONE && mission.prixFinal === mission.prixMin);
        if (!canConfirmFromStatus) {
            throw BusinessErrors.badRequest(`Le paiement final ne peut pas être confirmé. Statut actuel: ${mission.status}. ` +
...
//# sourceMappingURL=confirm-final-payment.usecase.js.map