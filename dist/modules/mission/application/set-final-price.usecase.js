...
export class SetFinalPriceUseCase {
    ...
    async execute(missionId, input) {
        ...
        if (mission.status !== MissionStatus.CONTACT_UNLOCKED) {
            throw BusinessErrors.badRequest(`Le prix final ne peut pas être fixé. Statut actuel: ${mission.status}. ` +
        ...
        if (input.prixFinal < mission.prixMin) {
            throw BusinessErrors.badRequest(`Le prix final (${input.prixFinal}) ne peut pas être inférieur au prix minimum (${mission.prixMin}).`);
        }
        if (input.prixFinal > mission.prixMax) {
            throw BusinessErrors.badRequest(`Le prix final (${input.prixFinal}) ne peut pas dépasser le prix maximum (${mission.prixMax}).`);
        }
...
//# sourceMappingURL=set-final-price.usecase.js.map