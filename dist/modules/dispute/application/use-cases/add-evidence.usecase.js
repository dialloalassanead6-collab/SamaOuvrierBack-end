...
export class AddEvidenceUseCase {
    ...
    async execute(input) {
        ...
        if (anyDispute.reporter_id !== input.uploadedBy && anyDispute.reported_user_id !== input.uploadedBy) {
            throw new DisputeDomainError('Vous n\'êtes pas autorisé à ajouter des preuves à cette dispute. Seuls les parties impliquées peuvent ajouter des preuves.');
        }
        ...
        if (anyDispute.status === 'RESOLVED' || anyDispute.status === 'CLOSED') {
            throw new DisputeDomainError('Impossible d\'ajouter des preuves à une dispute résolue ou fermée');
        }
        // 4. ✅ VALIDATION CÔTÉ SERVEUR - Vérifier type MIME
        if (!ALLOWED_MIME_TYPES.includes(input.file.mimetype)) {
            throw new DisputeDomainError(`Type de fichier non autorisé: ${input.file.mimetype}. Types autorisés: jpg, png, gif, webp, pdf, mp4, webm`);
        }
        // 5. ✅ LIMITE DE TAILLE - 10MB max
        if (input.file.size > MAX_FILE_SIZE) {
            throw new DisputeDomainError(`La taille du fichier dépasse la limite maximale de 10MB. Taille du fichier: ${(input.file.size / 1024 / 1024).toFixed(2)}MB`);
        }
        // 6. Upload file to Cloudinary with security settings
        const uploadResult = await cloudinaryService.uploadEvidence(input.file, input.disputeId, input.uploadedBy);
        ...
        const evidence = await this.disputeRepository.transaction(async () => {
            ...
            const createdEvidence = await this.disputeRepository.addEvidence(input.disputeId, evidenceInput);
            // Create audit event
...
//# sourceMappingURL=add-evidence.usecase.js.map