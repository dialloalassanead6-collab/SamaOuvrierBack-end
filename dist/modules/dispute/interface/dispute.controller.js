...
import { CreateDisputeUseCase, GetDisputesUseCase, ResolveDisputeUseCase, AddEvidenceUseCase, } from '../application/use-cases/index.js';
...
import { createDisputeSchema, getDisputesQuerySchema, resolveDisputeSchema, addEvidenceSchema, } from './dispute.validation.js';
...
export class DisputeController {
    ...
    async getDisputes(req, res) {
        try {
            ...
            if (validatedQuery.missionId)
                filters.missionId = validatedQuery.missionId;
            if (validatedQuery.reportedUserId)
                filters.reportedUserId = validatedQuery.reportedUserId;
    ...
    async getDisputeById(req, res) {
        try {
            ...
            const { id } = req.params;
            // Find the dispute
            const disputeRepo = this.createDisputeUseCase.disputeRepository;
    ...
    async deleteEvidence(req, res) {
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            if (evidence.dispute_id !== disputeId) {
                res.status(403).json({
                    success: false,
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            await cloudinaryService.deleteEvidence(evidence.public_id);
