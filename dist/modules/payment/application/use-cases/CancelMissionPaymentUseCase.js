...
export class CancelMissionPaymentUseCase {
     ...
     */
    async handleWorkerRefuse(input) {
        // Récupérer l'escrow
        ...
        // Sauvegarder
        await this.paymentRepository.updateAfterRefund(payment.id, escrow.id, updatedPayment, refundedEscrow);
     ...
     */
    async handleInProgressCancellation(input) {
        // Récupérer l'escrow
        const escrow = await this.paymentRepository.findEscrowByMissionId(input.missionId);
        const payment = await this.paymentRepository.findPaymentByMissionId(input.missionId);
        if (!escrow || !payment) {
        ...
        if (escrow.status !== EscrowStatus.HELD) {
            throw BusinessErrors.badRequest(`L'escrow ne peut pas être annulé. Statut actuel: ${escrow.status}`);
        }
        // Calculer le remboursement selon qui annule
        let clientRefundAmount;
        let workerAmount;
        ...
        const updatedPayment = this.escrowDomainService.markPaymentRefunded(payment);
        // Sauvegarder
        await this.paymentRepository.updateAfterRefund(payment.id, escrow.id, updatedPayment, escrow);
     ...
     */
    async handlePreInProgressCancellation(input) {
        // Récupérer l'escrow
        const escrow = await this.paymentRepository.findEscrowByMissionId(input.missionId);
        const payment = await this.paymentRepository.findPaymentByMissionId(input.missionId);
...
//# sourceMappingURL=CancelMissionPaymentUseCase.js.map