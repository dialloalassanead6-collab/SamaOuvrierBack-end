  ...
  async execute(input: CreatePaymentInput): Promise<CreatePaymentResult> {
    ...
    if (mission.status !== MissionStatus.PENDING_PAYMENT) {
      throw BusinessErrors.badRequest(
        `La mission n'est pas en attente de paiement. Statut actuel: ${mission.status}`
      );
    }
    ...
    const { payment, escrow } = this.escrowDomainService.holdFunds(
      input.missionId,
      input.clientId,
      mission.workerId,
      amount
    ...
    const paymentUrl = await this.paytechService.createPaymentRequest(
      amount,
      idempotencyKey,
      `Paiement mission ${input.missionId}`,
      {
