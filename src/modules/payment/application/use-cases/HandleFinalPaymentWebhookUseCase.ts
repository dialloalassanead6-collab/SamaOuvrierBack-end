...
export class HandleFinalPaymentWebhookUseCase {
   ...
   */
  async execute(input: HandleFinalPaymentWebhookInput): Promise<HandleFinalPaymentWebhookResult> {
    ...
    if (!escrow) {
      throw BusinessErrors.badRequest(
        'Escrow introuvable pour ce paiement. Ce n\'est pas un paiement final valide.'
      );
    }
  ...
  private async handleSuccess(
    ...
    _escrow: Awaited<ReturnType<IPaymentRepository['findEscrowByPaymentId']>>,
    _amount: number
  ): Promise<HandleFinalPaymentWebhookResult> {
    if (!payment) {
