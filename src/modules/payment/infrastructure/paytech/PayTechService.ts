...
export interface CreatePaymentPayload {
  ...
  item_price: number;
  currency: string;
  ref_command: string; // idempotency key
  command_name: string;
  env: string;
  ipn_url: string;
  success_url: string;
...
export class PayTechService {
  private readonly config: PayTechConfig;
   ...
   */
  async refund(amount: number, refCommand: string): Promise<boolean> {
   ...
   */
  private simulatePaymentRequest(payload: Record<string, unknown>): string {
   ...
   */
  private async callPayTechApi(payload: Record<string, unknown>): Promise<string> {
    const headers = {
 ...
 */
export function createPayTechService(): PayTechService {
  const config: PayTechConfig = {
    apiKey: process.env.PAYTECH_API_KEY || '',
    apiSecret: process.env.PAYTECH_API_SECRET || '',
    baseUrl: process.env.PAYTECH_BASE_URL || 'https://sandbox.paytech.sn/api',
    env: (process.env.PAYTECH_ENV as 'test' | 'production') || 'test',
