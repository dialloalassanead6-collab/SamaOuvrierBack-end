...
export interface EscrowConfig {
  commissionPercent: number; // Ex: 10 pour 10%
  currency: string;
}
...
export class EscrowDomainService {
   ...
   */
  releaseFunds(escrow: Escrow, releasedBy: string = 'SYSTEM'): ReleaseFundsResult {
    if (!escrow.isHeld()) {
      throw new Error(
        `Impossible de libérer les fonds. Statut actuel: ${escrow.status}. ` +
        `Attendu: HELD`
      );
    }
   ...
   */
  fullRefund(escrow: Escrow, releasedBy: string = 'SYSTEM'): Escrow {
    if (!escrow.isHeld()) {
      throw new Error(
        `Impossible de procéder au remboursement complet. Statut actuel: ${escrow.status}. ` +
        `Attendu: HELD`
      );
    }
  ...
  partialRefund(
    escrow: Escrow,
    requester: 'CLIENT' | 'WORKER'
  ): { escrow: Escrow; clientAmount: number; workerAmount: number } {
    if (!escrow.isHeld()) {
      throw new Error(
        `Impossible de procéder au remboursement partiel. Statut actuel: ${escrow.status}. ` +
        `Attendu: HELD`
      );
    }

    let clientRefundPercent: number;
    ...
    const refundedEscrow = escrow.partialRefund(
      clientRefundPercent,
      workerAmount,
      requester,
      releaseReason
   ...
   */
  private generateIdempotencyKey(missionId: string, type: string): string {
