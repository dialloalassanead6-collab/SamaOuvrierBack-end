...
import { Mission, type ProcessCancellationInput, type MissionResponse } from '../domain/index.js';
  ...
  async execute(
    ...
    if (!mission.isCancellationRequested()) {
      throw BusinessErrors.badRequest(
        `Aucune demande d'annulation en attente. Statut actuel: ${mission.status}.`
      );
    }
