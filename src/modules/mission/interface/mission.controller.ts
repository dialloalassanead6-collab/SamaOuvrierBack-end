...
import type { CreateMissionInput, SetFinalPriceInput, ProcessCancellationInput } from '../domain/index.js';
import {
  CreateMissionUseCase,
  ConfirmInitialPaymentUseCase,
  AcceptMissionUseCase,
  RefuseMissionUseCase,
  SetFinalPriceUseCase,
  ConfirmFinalPaymentUseCase,
...
export const missionController = {
  ...
  create: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    const { workerId, serviceId } = req.body as CreateMissionInput;
  ...
  list: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    ...
    const result = await getMissionsUseCase.execute(
      skip,
      take,
      clientId,
      workerId,
   ...
   */
  getById: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
   ...
   */
  confirmInitialPayment: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    const { id } = req.params as { id: string };
    const userId = req.user?.sub;
   ...
   */
  accept: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    const { id } = req.params as { id: string };
    const userId = req.user?.sub;
   ...
   * @requires Authentication (WORKER only)
   */
  refuse: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    const { id } = req.params as { id: string };
    const userId = req.user?.sub;
   ...
   * @requires Authentication
   */
  setFinalPrice: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    const { id } = req.params as { id: string };
    const { prixFinal } = req.body as SetFinalPriceInput;
    const userId = req.user?.sub;
   ...
   * Note: Cette endpoint est appelée par le webhook du système de paiement externe
   */
  confirmFinalPayment: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    const { id } = req.params as { id: string };
    const userId = req.user?.sub;
   ...
   * @requires Authentication
   */
  complete: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    const { id } = req.params as { id: string };
    const userId = req.user?.sub;
    const userRole = req.user?.role as 'CLIENT' | 'WORKER' | undefined;
   ...
   * @requires Authentication
   */
  requestCancellation: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    const { id } = req.params as { id: string };
    const userId = req.user?.sub;
    ...
    try {
      ...
      let cancellationRequester: 'CLIENT' | 'WORKER';
   ...
   * @requires Authentication
   */
  processCancellation: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    const { id } = req.params as { id: string };
    const { approved } = req.body as ProcessCancellationInput;
    const userId = req.user?.sub;
   ...
   * @requires Authentication
   */
  cancel: asyncHandler(async (req: Request, res: Response, _next: NextFunction): Promise<Response | void> => {
    const { id } = req.params as { id: string };
    const userId = req.user?.sub;
