...
import type { IDisputeRepository, CreateDisputeInput } from '../dispute.repository.interface.js';
...
export class CreateDisputeUseCase {
   ...
   */
  async execute(input: CreateDisputeInputDTO): Promise<CreateDisputeOutputDTO> {
    ...
    if (mission.clientId !== input.reporterId && mission.workerId !== input.reporterId) {
      throw new DisputeDomainError(
        'Vous ne pouvez pas ouvrir une dispute pour cette mission. Seuls le client et le worker peuvent ouvrir une dispute.'
      );
    }

    // 3. ✅ INTERDIRE DISPUTE SI MISSION FINALISÉE
    // Check mission status allows dispute
    if (!ELIGIBLE_MISSION_STATUSES_FOR_DISPUTE.includes(mission.status as typeof ELIGIBLE_MISSION_STATUSES_FOR_DISPUTE[number])) {
      throw new DisputeDomainError(
        `Impossible d'ouvrir une dispute pour une mission en statut: ${mission.status}. La mission doit être en cours, en attente de paiement final, ou en cours d'annulation.`
      );
    }
    ...
    if (activeDispute) {
      throw new DisputeDomainError(
        'Une dispute active existe déjà pour cette mission. Veuillez attendre la résolution de la dispute existante.'
      );
    }
    ...
    const existingDispute = await this.disputeRepository.findByMissionAndReporter(
      input.missionId,
      input.reporterId
    );
    ...
    const createdDispute = await this.disputeRepository.transaction(async () => {
      ...
      const createInput: CreateDisputeInput = {
        ...
        reportedUserId,
        reason: input.reason as import('@prisma/client').DisputeReason,
    ...
    const anyDispute = createdDispute as any;
