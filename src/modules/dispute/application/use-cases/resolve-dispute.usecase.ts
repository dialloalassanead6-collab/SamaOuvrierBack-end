...
export class ResolveDisputeUseCase {
   ...
   */
  async execute(input: ResolveDisputeInputDTO): Promise<ResolveDisputeOutputDTO> {
    ...
    if (anyDispute.status === DisputeStatus.RESOLVED || anyDispute.status === DisputeStatus.CLOSED) {
      throw new DisputeDomainError(
        'Cette dispute a déjà été résolue ou fermée. Impossible de la résoudre à nouveau.'
      );
    }
    ...
    if (anyDispute.status !== DisputeStatus.UNDER_REVIEW) {
      throw new DisputeDomainError(
        `La dispute ne peut pas être résolue dans son état actuel: ${anyDispute.status}. Elle doit être mise sous examen avant résolution.`
      );
    }
    ...
    const resolvedDispute = await this.disputeRepository.transaction(async () => {
      ...
      await this.handleEscrowResolution(
        anyDispute.mission_id,
        input.resolution
      );
    ...
    // 5. Return result
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const anyResolved = resolvedDispute as any;
   ...
   */
  private async handleEscrowResolution(missionId: string, resolution: string): Promise<void> {
