...
export interface NotificationAdditionalData {
  // Pour les notifications de compte rejeté
  rejectionReason?: string;
  
  // Pour les notifications de mission
  workerName?: string;
  clientName?: string;
  serviceTitle?: string;
  
  // Pour les notifications de dispute
  reason?: string;
  resolution?: string;
  
  // Pour les notifications de review
  reviewerName?: string;
  rating?: number;
...
export class CreateNotificationUseCase {
   ...
   */
  async execute(input: CreateNotificationInput): Promise<NotificationEntity> {
   ...
   */
  private getMessages(type: NotificationType): { title: string; message: string } {
  ...
  async createAccountNotification(
    userId: string,
    type: NotificationType.ACCOUNT_CREATED | NotificationType.ACCOUNT_PENDING_APPROVAL | NotificationType.ACCOUNT_APPROVED | NotificationType.ACCOUNT_REJECTED,
   ...
   * @returns La notification créée
   */
  async createMissionNotification(
    userId: string,
  ...
  async createDisputeNotification(
    userId: string,
    type: NotificationType.DISPUTE_OPENED | NotificationType.DISPUTE_STATUS_UPDATED | NotificationType.DISPUTE_RESOLVED,
    disputeId: string,
    additionalData?: NotificationAdditionalData
  ): Promise<NotificationEntity> {
    const baseMessages = this.getMessages(type);
  ...
  async createPaymentNotification(
    userId: string,
    type: NotificationType.PAYMENT_RECEIVED | NotificationType.PAYMENT_RELEASED | NotificationType.PAYMENT_REFUNDED,
    paymentId: string,
    additionalData?: NotificationAdditionalData
  ): Promise<NotificationEntity> {
    const baseMessages = this.getMessages(type);
